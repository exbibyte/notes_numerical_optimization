...
\begin{multicols*}{2}
  ...
  Example: $w^k = \frac{k}{k+3}, w^0 = 0, \lambda^k \in (0,1/L], L:= \text{Lipschitz constant of }\nabla f,\ or\ \lambda^k$ found via line search.
    ...
    \begin{algorithm}[H]
      ...
      \For{$col\ in\ 0\ ..\ n$}{
        let $rowmax \leftarrow $col\\
        \For{$row\ in\ col+1\ ..\ n$}{
          \uIf{$A_{row, col} > A_{rowmax,col}$}{
            $rowmax \leftarrow row$\\
          }
        }
        
        swap($A_{rowmax, :}$, $A_{col, :}$)\\
        swap($P_{rowmax}$, $P_{col}$)\\
        $S \leftarrow S +  rowmax\ !=\ col\ ?\ 1\ :\ 0$\\

        \uIf{$A_{col, col} < \epsilon_{tol}$}{
          raise\ degenerate\ matrix\ error
        }
        \For{$row\ in\ col+1\ ..\ n$}{
          $r \leftarrow A_{row, col} / A_{col,col}$\\
          $A_{row, col} \leftarrow r$ //save value for $L$\\
          //reduction for $U$\\
          \For{$c\ in\ col+1\ ..\ n$}{
            $A_{row,c} \leftarrow A_{row,c} - r A_{col, c}$\\
          }
        }
      }
      return $A, P, S$
    ...
    \begin{algorithm}[H]
      ...
      \For{$k=0..$}{
        $y_{k+1} = A x_k$\\
        $x_{k+1} = \frac{y_{k+1}}{\|y_{k+1}\|_{\infty}}$ //normalize
      }\\
    ...
    \begin{algorithm}[H]
      let $x_0 \leftarrow ...$\\
      \For{$k=0..$}{
        Solve for $y_{k+1}$ with factorization:\\
        $A y_{k+1} = x_k$\\
        $x_{k+1} = \frac{y_{k+1}}{\|y_{k+1}\|_{\infty}}$ //normalize
      }\\
    ...
    \begin{algorithm}[H]
      ...
      \For{$k=0..$}{
        $\sigma_{k+1} = \frac{x_k^T A x_k}{x_k^T x_k}$//shift\\
        solve $y_{k+1}$: $(A-\sigma_{k+1}I)y_{k+1} = x_k$//next vector iterate\\
        $x_{k+1} = \frac{y_{k+1}}{\|y_{k+1}\|_{\infty}}$//normalize\\
      }\\
